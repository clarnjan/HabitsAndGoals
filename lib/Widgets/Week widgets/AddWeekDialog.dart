...
class AddWeekDialog extends StatefulWidget {
  ...
  const AddWeekDialog({Key? key, this.task, required this.refreshParent}) : super(key: key);
...
class _AddWeekDialogState extends State<AddWeekDialog> {
  ...
  pickEndDate() async {
    ...
    final newDate = await showDatePicker(
        context: context, initialDate: DateTime.now(), firstDate: DateTime(DateTime.now().year - 1), lastDate: DateTime(DateTime.now().year + 1));
    setState(() {
      endDate = newDate;
    });
  }
        ...
        child: Column(
          ...
          children: [
            ...
            TextFormField(
              ...
              decoration: InputDecoration(
                hintText: startDate != null ? DateService.formatDate(startDate!) : "Select start date",
              ),
            ),
          ],
        ),
        ...
        TextButton(
          ...
          onPressed: () async {
            if (formKey.currentState!.validate()) {
              final week = Week(title: title, startDate: startDate!, endDate: endDate!);
